...
use crate::{Error, read_file, Result};
...
fn cpu_get_cpuinfo_freq() -> Result<Vec<FloatCount>> {
	...
	contents
		...
		.map(|line| {
			line.split(':').last().unwrap_or("0.0").trim()
				.parse::<FloatCount>().map_err(|err| Error::ParseFloat {
...
pub fn cpu_freq_percpu() -> io::Result<Vec<CpuFreq>> {
	...
	let cpuinfo_freqs = cpu_get_cpuinfo_freq().unwrap_or(vec![]);
	...
	let mut paths: Vec<PathBuf> = glob("/sys/devices/system/cpu/cpufreq/policy[0-9]*")
		...
		.map(|path| path.expect("I'm not sure why this would ever fail")).collect();
	...
	for path in paths {
		let path_string = path.clone().into_os_string().into_string().expect("Path string was not valid utf-8");
		let cpu_num = path_string.chars().filter(|c| c.is_digit(10)).collect::<String>().parse::<usize>();
		if cpu_num.is_ok() {
			...
			Some(path) => {
				let curr = if cpuinfo_freqs_valid {
					...
					read_file(path.join("scaling_cur_freq"))
						.unwrap_or_else(|_| read_file(path.join("cpuinfo_cur_freq"))
				...
				let max = read_file(path.join("scaling_max_freq")).unwrap().trim().parse::<f64>().unwrap() / 1000.0;
				let min = read_file(path.join("scaling_min_freq")).unwrap().trim().parse::<f64>().unwrap() / 1000.0;
